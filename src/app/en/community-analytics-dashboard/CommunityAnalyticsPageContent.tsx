...
export function CommunityAnalyticsPageContent() {
  const { t, language } = useLanguage();
  const pageT = t.pages.communityAnalyticsDashboard;
  ...
  return (
    <>
      <Script
        id="faq-schema-community-analytics-en"
        ...
        <section className="py-12 md:py-16 bg-white border-t border-gray-100">
          <div className="container mx-auto px-4 sm:px-6 md:px-12 max-w-4xl">
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-4">
              <Link href={getLocalizedPath('/discord-analytics', language)} className="text-[#214BCE] hover:text-[#6C2BD9] font-medium underline underline-offset-2">
                {language === 'ja' ? 'Discord分析を見る →' : 'View Discord Analytics →'}
              </Link>
              <span className="text-gray-300">|</span>
              <Link href={getLocalizedPath('/membership-retention', language)} className="text-[#214BCE] hover:text-[#6C2BD9] font-medium underline underline-offset-2">
                {language === 'ja' ? '継続率分析を見る →' : 'View Retention Analysis →'}
              </Link>
            </div>
          </div>
        </section>
